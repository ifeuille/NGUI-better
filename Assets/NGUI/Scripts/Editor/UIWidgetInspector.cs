...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public void DrawInspectorProperties (SerializedObject so, UIWidget w, bool drawColor)
	{
		...
		if (NGUIEditorTools.DrawHeader("Widget"))
		{
            ...
            DrawExtraPropties(so, w, type == PrefabType.Prefab);
    ...
    static void DrawExtraPropties(SerializedObject so, UIWidget w, bool isPrefab)
    {        
        // Pivot point -- the new, more visual style
        GUILayout.BeginHorizontal();
        GUILayout.Label("层级");
        EditorGUILayout.IntField(w.mLayer);
        GUILayout.EndHorizontal();
        GUILayout.BeginHorizontal();
        GUILayout.Label("功能");
        w.mIsFunction = GUILayout.Toggle(w.mIsFunction, "功能");
        GUILayout.EndHorizontal();        
    }
