...
public class UIPanel : UIRect
{
	...
	public override Vector3[] GetSides (Transform relativeTo, UIRect.EnumSideType SideType = UIRect.EnumSideType.AdaptTwoSide)
	{
		...
		return base.GetSides(relativeTo, SideType);
	}
	...
	void UpdateSelf ()
	{
		...
		if (mRebuild)
		{
			...
			FillAllDrawCalls();
#if UNITY_EDITOR
            Debug.LogWarningFormat("FillAllDrawCalls calld [{0}]", this.name);
#endif
        }
        else
		{
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				if (mat != null || sdr != null || tex != null)
				{
					...
					else
					{
                        ...
                        else if (rd == dc.depthStart) ++dc.depthStartCount;
						if (rd > dc.depthEnd) dc.depthEnd = rd;
                        else if (rd == dc.depthEnd) ++dc.depthEndCount;
                    }
	...
	public UIDrawCall FindDrawCall (UIWidget w)
	{
		...
		int depth = w.depth;
        if (mat == null && tex == null) return null;
        for (int i = 0; i < drawCalls.Count; ++i)
		{
			UIDrawCall dc = drawCalls[i];
			...
			if (dcStart <= depth && dcEnd >= depth)
			{
				if (dc.baseMaterial == mat && dc.mainTexture == tex)
				{
					if (w.isVisible)
					{
                        if (depth == dc.depthStart) ++dc.depthStartCount;
                        if (depth == dc.depthEnd) ++dc.depthEndCount;
                        w.drawCall = dc;
	...
	public void RemoveWidget (UIWidget w)
	{
		...
		{
			int depth = w.depth;
            if(depth == w.drawCall.depthStart)
            {
                --w.drawCall.depthStartCount;
            }
            if (depth == w.drawCall.depthEnd)
            {
                --w.drawCall.depthEndCount;
            }
            //if (depth == w.drawCall.depthStart || depth == w.drawCall.depthEnd)
            if(w.drawCall.depthStartCount <= 0 || w.drawCall.depthEnd <= 0)
				mRebuild = true;
